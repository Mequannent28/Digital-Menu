...
router.put('/groups/:id', auth, async (req, res) => {
  try {
    const { name, name_am, required, multi_select, max_select } = req.body
    const r = await query(
      `UPDATE modifier_groups SET name=@name, name_am=@nameAm, required=@req, multi_select=@multi, max_select=@max
       OUTPUT INSERTED.*
       WHERE id=@id`,
      {
        name: { type: sql.NVarChar, value: name },
...
router.delete('/groups/:id', auth, async (req, res) => {
  try {
    await query(`DELETE FROM modifier_groups WHERE id=@id`, {
      id: { type: sql.Int, value: parseInt(req.params.id) }
    })
    res.status(204).end()
  } catch (err) { res.status(500).json({ error: err.message }) }
...
router.delete('/items/:id', auth, async (req, res) => {
  try {
    await query(`DELETE FROM modifiers WHERE id=@id`, {
      id: { type: sql.Int, value: parseInt(req.params.id) }
    })
    res.status(204).end()
  } catch (err) { res.status(500).json({ error: err.message }) }
